		...
		if (valido) {
			...
			for (int i = 0; i < evaluacion.size(); i++) { 
				...
				if (multiple.getNegacion()) {
					System.out.println("Nivel: " + multiple.getNivel() + " " +  multiple.getIdCondicion() + " (NOT " + multiple.getTexto() + ") Madre: " +  multiple.getMadre() + " Tipo: " + multiple.getTipo() + " NegaciÃ³n: " + multiple.getNegacion());
				} else {
					System.out.println("Nivel: " + multiple.getNivel() + " " +  multiple.getIdCondicion() + " " + multiple.getTexto() + " Madre: " +  multiple.getMadre() + " Tipo: " + multiple.getTipo() + " NegaciÃ³n: " + multiple.getNegacion());
				}
...
//				simples = parte.split(opLogicos[1]);
	...
	private String formatoCondicion(String cadena) {
		...
		// Se aÃ±ade un espacio delante y detrÃ¡s de cada uno de los operadores.
		for (int i = 0; i< operadores.length;i++) {
			cadenaFormato = cadenaFormato.replaceAll(operadores[i].toLowerCase(), " " + operadores[i] + " ");			
		}		
		// Se elimina el espacio que se habrÃ­a aÃ±adido en el bucle anterioren los operadores que se definen con dos caracteres
		...
		// Convierte a mayÃºsculas todos los operadores AND y OR
		...
		cadenaFormato = cadenaFormato.replaceAll("\\(not \\( ", "( NOT \\(");
		cadenaFormato = cadenaFormato.replaceAll("\\ NOT \\( ", "\\( NOT \\(");
		// Se eliminan los espacios de mÃ¡s	
		cadenaFormato = cadenaFormato.replaceAll("\\(", " \\(");
		cadenaFormato = cadenaFormato.replaceAll("\\) ", "\\) ");
...
//		remplazo = remplazarCadena(remplazo, "  ", " ");
	...
	// Comprueba si la informaciÃ³n obtenida para el valor del opernado es completa y coherente.
			...
			if (operando.startsWith(constantes.comillas) && operando.endsWith(constantes.comillas) && contarCaracter(operando, constantes.comillas) == 2) {
				...
				if (contarCaracter(operando, constantes.comillas) == 0) {
					...
					try {
						...
						nuevoError("El operando contiene un valor numÃ©rico errÃ³neo: " + operando);
					}					
				}
			}
		}		
	...
	int compara;
		...
		if (simple.getEvaluada()) {return simple.getResultado();  } 
			...
			// Si ha podido convertir a int ambos resultados, los compararÃ¡ numÃ©ricamente. Si no, harÃ¡ comparaciÃ³n alfabÃ©ticamente.
