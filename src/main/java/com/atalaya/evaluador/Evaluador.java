...
public class Evaluador {
	...
	public StringBuffer evaluarAnalisis() {
		...
		// Lo primero que voy a hacer es traerme las configuraciones
		// Ahora mismo solo vamos a tener configuraciones para los hilos.
		// Si en el analisis no viaja configuraciones de hilos suponemos que el usuario quiere monohilo
		if (analisis.getAnalisis().getConfiguraciones().size() == 0) {
			log.info("Sin información de configuracion de Thread. Ajustamos a MONOHILO");
			num_threads_max = 1;
		
		}else {
			
			for (int c=0; c < analisis.getAnalisis().getConfiguraciones().size(); c++ ) {
				
				Configuracion conf = analisis.getAnalisis().getConfiguraciones().get(c);
				
				String nombreConfiguracion = 	  conf.getNombre();
				String descripcionConfiguracion = conf.getDescripcion();
				ArrayList<Parametro> parametros = conf.getParametros();
				
				if (nombreConfiguracion.equals("Threads")) {

					//Me traigo la configuracion de si vamos a hacerlo multihilo
					Parametro parametro1 = parametros.get(0);
					
					if (parametro1.getNombre().equals("MultiHilo") && (parametro1.getValor().equals("Si"))) { //esta es la configuarión para hacerla multihilo
						//Me traigo la configuracion del numero de hilos en el caso de que sea multihilo
						Parametro parametro2 = parametros.get(1);
						
						if ( parametro2.getNombre().equals("NumeroHilos") ) {
							try {
								num_threads_max = Integer.parseInt(parametro2.getValor());
								log.info("ESTAMOS EN MODO MULTIHILO CON  " + num_threads_max + "  HILOS EN DISPOSICION....");
							} catch (Exception e) {
								log.info("El valor pasado como numero de hilos NO es un valor válido. Se deja como MONOHILO");
								num_threads_max = 1;	
							}
						}else {
							num_threads_max = 1;
					...
					}else { //Ponga lo que ponga lo tomo como MONOHILO
						log.info("Sin información de configuracion de Thread. Ajustamos a MONOHILO");
						num_threads_max = 1;
					}
					
				}//De aquí irán colgando las diferentes configuraciones venideras
				
			}
		} //Finaliza el control de la configuracion
...
//		}
	...
	// Recorre el texto de la condicion indicada, identificando por niveles el texto comprendido entre parentesis. Cada trozo de texto comprendido entre parentesis del mismo nivel, será una condicion
		...
		CondicionMultiple multiple;		
		log.info("condicion completa a evaluar: " + this.condicionCompleta);
		char c;
	...
	private boolean interpretarOperando(Operando op)
	{
		...
		if (!op.getEjecutado())
		{
			if (op.getTipo() == Comunes.tpIndicador) 
			{
				...
				String nombreIndicador = tramos[0].substring(1);
				log.info("Interpretando operando de tipo indicador: "+nombreIndicador);
				...
				if (indicador.noejecutado()) 
				{
					...
					{	
							...
							for (int i=0; i<listaIndicadoreAsociado.size(); i++)
							{
								...
								if (num_threads_max - nThreadsOcupados>0)
								{
									...
									{ 
										...
										log.info("Arranco nuevo hilo para el indicador " + indicadorAsociado.getIndicador().getNombre() + " | " + indicadorAsociado.getEstado() + " | " + indicadorAsociado.toString() + " | en la posicion " + nPosicionThreadLibre);
									}
									else if (indicadorAsociado!=null && indicadorAsociado.ejecutado() && indicadorAsociado.getResultadoEjecucion()!=null && indicadorAsociado.getResultadoEjecucion().size()==0)
									{
										...
										log.info("El indicador asociado " + indicadorAsociado.getIndicador().getNombre() + " ha finalizado.");
										nThreadsOcupados = 0;
										for(int j=0; j<threads.length;j++)
										{
											if (threads[j]!=null && threads[j].isAlive())
										...
										log.info("El numero de hilos ocupados es: "+nThreadsOcupados);
									}
								...
								{
									log.info("No hay hilos disponibles. Esperamos.");
									try {
										Thread.sleep(5000);
									} catch (InterruptedException e) {
										// TODO Auto-generated catch block
						...
						log.info("El indicador " + indicador.getIndicador().getNombre() + " ha finalizado.");
